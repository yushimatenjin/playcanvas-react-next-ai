...
export default function CanvasPage() {
	...
	const { sections, currentSectionIndex, scrollProgress, sectionProgress } =
	...
	const getInterpolatedEffects = (
		preset: (typeof visualPresets)[0],
		progress: number,
	): Partial<PostEffectsSettings> => {
		...
		if (effects.bloom) {
			...
			effects.bloom.lastMipLevel = Math.max(
				2,
				Math.floor(3 + Math.sin(progress * Math.PI) * 1),
			);
		}
		...
		if (effects.vignette) {
			...
			effects.vignette.curvature =
				(effects.vignette.curvature || 0) *
				(1 + Math.sin(progress * Math.PI) * 0.2);
		}

		if (effects.grading) {
			// カラーグレーディングの動的な調整
			effects.grading.saturation =
				(effects.grading.saturation || 1) *
				(1 + Math.sin(progress * Math.PI * 2) * 0.15);
			effects.grading.contrast =
				(effects.grading.contrast || 1) *
				(1 + Math.sin(progress * Math.PI) * 0.1);
			effects.grading.brightness =
				(effects.grading.brightness || 1) *
	...
	return (
		...
		>
			{sections.map((sectionIndex) => {
				...
				return (
					<section key={sectionIndex} className="h-screen w-full sticky top-0">
						<SceneCanvas
